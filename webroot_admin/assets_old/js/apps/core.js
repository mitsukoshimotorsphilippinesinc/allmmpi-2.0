...
$(document).ready(function() {
	...
	$("body").on("click", "a.facility_selector", function(e) {
		e.preventDefault();
		var facility_id = $(this).attr('data-id');

		b.request({
			url: '/admin/select_facility',
	...
	$(document).on("focus",".keypad_qty",function(){
		$(this).keyboard({
			layout: 'custom',
			customLayout: {
				'default': [
	...
	var poiQuantity = function(entry,quantity,type,callback)
	{
		...
		var poi_qty_btn = {
			...
			"Ok" : function(){
				var qty = $("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").val();

				if(_.isEmpty(qty))
				{
					alert("Please specify the quantity");
					$("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").focus();
					return false;
				}
				...
				if(_.isFunction(callback)){
					...
					{
						callback.call(this,{"product_id":entry.id,"quantity":qty});
					}
					else if(entry.type == "pack")
					{
						callback.call(this,{"package_id":entry.id,"quantity":qty});
					}
				}
				close_poi_qty(true);
			}
		...
		if(!_.isEqual(cart.discount,{}) && type == "rebate")
		{
			extra_btn = {
				"Use Product Rebate": function(){
					var qty = $("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").val();
					callback.call(this,{"product_id":entry.id,"quantity":qty,"type":"rebate"});
					close_poi_qty(true);
				}
			};
			$.extend(poi_qty_btn,extra_btn);
		}
		
		
		
		function close_poi_qty(accept){
			var keyboard = $("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").getkeyboard();
			keyboard.options.stayOpen = false;
			keyboard.options.alwaysOpen = false;
			keyboard.close(accept);
			keyboard.destroy();
			poi_qty_modal.hide();
		}

		
		var poi_qty_modal = b.modal.new({
			title: "Quantity for "+entry.name,
			width: 354,
			disableClose: true,
			html: '\
					<div class="keypad">\n\
						<input class="keypad_qty" type="text" name="keypad_qty" style="width:204px; text-align:right;" maxlength="10" value="'+quantity+'">&nbsp;<a id="poi_keyboard" class="btn btn-large"><img src="/assets/img/keyboard.png"></a>\n\
					</div>',
			buttons: poi_qty_btn
		});
	...
	poi_qty_modal.show();
		...
		$("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").focus();
		$("#"+$(poi_qty_modal).attr("id")).find(".keypad_qty").focus();
		$(".ui-keyboard:last").css("display","none");
	}

	$(document).on("click","#poi_keyboard",function(){
		var keyboard = $(".keypad_qty:last").getkeyboard();
		
		if(!keyboard.isVisible())
		{
			keyboard.reveal();
			keyboard.options.stayOpen = true;
