...
class Notifications extends  Base_Controller
{
	...
	public function send_email($data=array())
	{
		...
		// 20141021 - DISABLE ELASTIC EMAIL
		// START
	...
	public function send_tb_sms($data=array())
	{
		...
		if($this->settings->sms_use_sun_api){
			...
			if(!$ret) {
                ...
                // update the sms_sent on credit logs
                $this->load->model("tracking_model");
                
                $where = "credit_log_id = " . $data["log_id"];
                $data = array(
                		sms_sent => 1,
                		tariff => $data["tariff"]
                	);
                
                $this->tracking_model->update_acct_credit_logs();
            }    
	...
	public function send_email_batch($data=array())
	{
		// load contents_model
		$this->load->model("contents_model");
		...
		$batch_email_addresses = $data['batch_email_addresses']; // semicolon delimited
		
		$email_addresses = explode("|", $batch_email_addresses);
