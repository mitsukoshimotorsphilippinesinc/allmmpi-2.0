...
class Navigation extends Admin_Controller {
	...
	public function index($filter = '')
	{
		...
		$navigations = $this->navigation_model->get_navigation(null,null,'priority_order ASC');
		//$inv_max_order = $this->navigation_model->get_system_max_order('inventory');
		//$net_max_order = $this->navigation_model->get_system_max_order('network');
		//$pos_max_order = $this->navigation_model->get_system_max_order('webpoi');
		//$sys_max_order = $this->navigation_model->get_system_max_order('system');
		//$site_max_order = $this->navigation_model->get_system_max_order('site');
		//$cms_max_order = $this->navigation_model->get_system_max_order('cms');
		//$mis_max_order = $this->navigation_model->get_system_max_order('mis');
	...
	{
		/*$navigation = $this->navigation_model->get_navigation_by_id($navigation_id);

		$parent = $this->navigation_model->get_navigation_by_id($navigation->parent_id);
		$parent_title = "";
		if(!empty($parent))
		{
	...
	public function add()
	{
		...
		/*if ($_POST)
		{
			...
			if ($this->form_validation->run())
			{
				...
				$max_order = $this->navigation_model->get_system_max_order(set_value('system_code'));
				...
				$this->navigation_model->insert_navigation($data);
	...
	public function confirm_add() {
		...
		if ($_parent_id == 0) {
			...
			$navigation_details = $this->navigation_model->get_navigation_by_id($_parent_id);
	...
	public function add_navigation() {
		...
		$max_order = $this->navigation_model->get_system_max_order($_system_code);
		...
		$insert_id = $this->navigation_model->insert_id();
	...
	public function edit()
	{
		...
		$navigation = $this->navigation_model->get_navigation_by_id($_navigation_id);
		...
		/*if ($_POST && !empty($navigation))
		{
			...
			if ($this->form_validation->run())
			{
				...
				{
					$max_order = $this->navigation_model->get_system_max_order($post_system_code);

					$ordering = $max_order + 1;

					if($navigation->ordering != $max_order)
					{
						$other_nav = $this->navigation_model->get_navigation_by_system_and_ordering($orig_system_code,$navigation->ordering+1);
						...
						while(!empty($other_nav))
						{
							...
							$this->navigation_model->update_navigations($data, array('navigation_id' => $other_nav->navigation_id));
							$other_nav = $this->navigation_model->get_navigation_by_system_and_ordering($orig_system_code,$other_nav->ordering+1);
						}
					}
				}
				...
				$this->navigation_model->update_navigations($data, array('navigation_id' => $navigation_id));
	...
	public function update_navigation() {
		...
		$this->navigation_model->update_navigations($data, array('navigation_id' => $_navigation_id));
	...
	public function delete()
	{
		...
		/*if ($_POST && !empty($navigation))
		{
			...
			if (!empty($_navigation_id)) if ($_navigation_id == $navigation_id)
			{
				...
				$max_order = $this->navigation_model->get_system_max_order($system);
				
				if($navigation->ordering != $max_order)
				{
					$other_nav = $this->navigation_model->get_navigation_by_system_and_ordering($system,$navigation->ordering+1);
					...
					while(!empty($other_nav))
					{
						...
						$this->navigation_model->update_navigations($data, array('navigation_id' => $other_nav->navigation_id));
						$other_nav = $this->navigation_model->get_navigation_by_system_and_ordering($system,$other_nav->ordering+1);
					}
				}

				$this->navigation_model->delete_navigations(array('navigation_id' => $navigation_id));
	...
	public function delete_navigation() {	
		...
		if (!empty($_navigation_id)) {		
			...
			$this->navigation_model->delete_navigations(array('navigation_id' => $_navigation_id));
	...
	public function order($system,$direction,$navigation_id)
	{
		...
		{
			$max_order = $this->navigation_model->get_system_max_order($system);
			$sec = $this->navigation_model->get_navigation_by_id($navigation_id);
			...
			if ($_other_order > 0)
			{
				$_other_sec = $this->navigation_model->get_navigation_by_system_and_ordering($system,$_other_order);
				$this->navigation_model->update_navigations(array('ordering' => $_other_sec->ordering), array('navigation_id' => $sec->navigation_id));
				...
				$this->navigation_model->update_navigations(array('ordering' => $sec->ordering), array('navigation_id' => $_other_sec->navigation_id));
	...
	public function get_headers()
	{
		...
		$section_headers = $this->navigation_model->get_navigation_headers_by_system_code($system_code);
