<div class="alert alert-info">
	<h2>Edit FAQ  <a href='/cms/faqs' class='btn btn-small' style="float:right;margin-top:5px;margin-right:-30px;">Back</a></h2>
</div>
<hr/>
<?php if (empty($faq)): ?>
<h3>FAQ not found.</h3>
...
<form action='/cms/faqs/edit/<?= $faq->faqs_id; ?>' method='post' class='form-horizontal'>
	<fieldset >
		<div class="control-group <?= $this->form_validation->error_class('question') ?>">
			<label class="control-label" for="promo_title">Question <em>*</em></label>
			<div class="controls">
				<input type="text" class='span6' placeholder="" name="question" id="question" value="<?= set_value('question',$faq->question) ?>">
				<p class="help-block"><?= $this->form_validation->error('question'); ?></p>
			</div>
		</div>
		<div class="control-group <?= $this->form_validation->error_class('body') ?>">
			...
			<div class="controls">
				<div class=''>
					<textarea name="body" id="body" class="input-xlarge span10" rows="20"><?= $this->form_validation->set_value('body',$faq->body) ?></textarea>
				</div>
		...
		<div class="control-group <?= $this->form_validation->error_class('is_published') ?>">
			...
			<div class="controls">
				...
				echo form_dropdown("is_published", $options, set_value('is_published',$faq->is_published),"id='is_published' style='width:auto;'");
