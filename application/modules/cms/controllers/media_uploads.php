...
class Media_uploads extends CMS_Controller
{
	...
	function __construct()
	{
		...
		$this->set_navigation('media_uploads');
	}
	...
	public function media_uploads()
	{
		...
		// set pagination data
		$config = array(
		    'pagination_url' => "/cms/media_uploads/index",
		    'total_items' => $this->contents_model->get_media_uploads_count($where),
		    'per_page' => 10,
		    'uri_segment' => 4
		);
		...
		$this->template->media_uploads = $this->contents_model->get_media_uploads($where, array('rows' => $this->pager->per_page, 'offset' => $this->pager->offset), 'insert_timestamp DESC');
		$this->template->view('media_uploads/list');
	}
	...
	public function add()
	{
		if ($_POST)
		{
			...
			if ($this->form_validation->run())
			{
				...
				$this->contents_model->insert_media_uploads($data);
				...
				redirect('/cms/media_uploads');
				return;
			}
		}
		
		$this->template->view('media_uploads/add');
	}
	
	public function edit($media_upload_id = 0)
	{
		$media_upload = $this->contents_model->get_media_uploads_by_id($media_upload_id);
		...
		if ($_POST and !empty($media_upload))
		{
			...
			if ($this->form_validation->run())
			{
				...
				$this->contents_model->update_media_uploads($data, array('media_upload_id' => $media_upload_id));
	...
	public function delete($media_upload_id = 0)
	{
		...
		if ($_POST and !empty($media_upload))
		{
			...
			{
				$this->contents_model->delete_media_uploads(array('media_upload_id' => $media_upload_id));
