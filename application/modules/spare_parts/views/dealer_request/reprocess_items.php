...
$submitURL = "/spare_parts/" . $department_module_details->segment_name . "/reprocess/" . $dealer_request_details->dealer_request_id;
...
<?php
	...
	$details_content = get_requester_details($dealer_request_details->id_number, "employee", 1);
	...
	<table  class='table table-bordered'>
		...
		<tbody>
			...
			<tr>
				...
				<?php
					if (strlen(trim($dealer_request_details->remarks)) > 0)
						echo "<td><a href='#' id='view-full-remarks' data='{$dealer_request_details->remarks}'><u>View Remarks</u></a></td>";
					else
...
<form id="submit_form" action='<?= $submitURL ?>' method='post' class='form-inline'>
	<fieldset >		
						...
						<td>
							<?php
								...
								foreach ($request_items as $ri) {
									...
									$item_detail_options[$ri->dealer_request_detail_id] = $item_view_details->sku . ' - ' . $item_view_details->description;
			...
			<table class="table inventory-order-items table-bordered table-striped">
				...
				<tbody id="wr_items">
					...
					<tr class="item_row">
						...
						<td class ="remove_item" id="<?= $temp[$i]['request_item_id'] ?>" data="<?= $temp[$i]['dealer_request_detail_id'] ?>" data-id="<?= $temp[$i]['request_item_id'] ?>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>						
		...
		<div class="controls" align="right">
			...
			<a href='/spare_parts/display_returnslip/<?= $dealer_request_details->request_code ?>' target = '_blank' id='print_return_slip' class="btn btn-primary"><span id="prs_caption">Print Return Slip Form</span></a>						
...
<script type="text/javascript">
	...
	var item_entry_row = _.template('\
		...
		<td id="<%= active_reprocessed_item_id %>" data="<%= active_dealer_request_detail_id %>" data-id="<%= active_reprocessed_item_id %>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>\n\
		<td class="hidden_values">\n\
	...
	$("#add_wr_item").live('click',function(){
		...
		if (hasError == 0) {
			...
			// ajax request
			b.request({
				url : '/spare_parts/dealer_request/proceed_reprocess_item',
				data : {				
	...
	$("#view-full-remarks").click(function(e){
		...
		var request_code = '<?= $dealer_request_details->request_code ?>';
	...
	$("#forward_to_warehouse").live('click',function(){
		...
		var request_code = '<?= $dealer_request_details->request_code ?>';
		var segment_name = '<?= $department_module_details->segment_name ?>';
		var request_id   = <?= $dealer_request_details->dealer_request_id ?>;
