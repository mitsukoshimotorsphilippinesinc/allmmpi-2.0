<fieldset >		
		...
		<table  class='table table-bordered'>
			...
			<tbody>
				<tr>
					...
					<td><?= $warehouse_request->request_code; ?></td>
					<td><strong>Engine:</strong></td>
					<td><?= $warehouse_request->engine; ?></td>
				</tr>	
				<tr>
					<td><strong>Requestor:</strong></td>
					<?php
					$requestor_details = $this->human_relations_model->get_employment_information_by_id($warehouse_request->id_number);
					...
					<td><?= $warehouse_request->chassis; ?></td>
				</tr>
				<tr>
					<td><strong>Status:</strong></td>
					<td><?= $warehouse_request->status; ?></td>
					<td><strong>Brand/Model:</strong></td>					
					<?php
					$motor_brand_model_details = $this->warehouse_model->get_motorcycle_brand_model_class_view_by_id($warehouse_request->motorcycle_brand_model_id);				
				...
				<tr>
					...
					<?php
					$warehouse_details = $this->spare_parts_model->get_warehouse_by_id($warehouse_request->warehouse_id);
					...
					<?php
					$warehouse_approvedby_details = $this->human_relations_model->get_employment_information_by_id($warehouse_request->warehouse_approved_by);
	...
	<div>
		<table  class='table table-striped table-bordered'>
			...
			<tbody>
				<?php if(empty($warehouse_request_details)):?>
					<tr><td colspan='9' style='text-align:center;'><strong>No Record Found</strong></td></tr>
				<?php else: ?>
				<?php foreach ($warehouse_request_details as $wrd): ?>
				<tr>
					<td><?= $wrd->item_id; ?></td>
					<td><?= $wrd->srp; ?></td>
					<td><?= $wrd->discount; ?></td>
