...
if (is_object($service_unit_details))
{
	$isAdd = false;
	$titlePrefix = "Edit&nbsp;";
	...
	<fieldset >
		<div class="row-fluid">						
			...
			<?php if(!$isAdd && !empty($service_unit_details)):?>
				<label class="label label-important request-code-label" id="requester-request-code-label" style="float:right;font-size:16px;"><?= $service_unit_details->request_code ?></label>
			<?php elseif($isAdd): ?>
		...
		<div class="row-fluid">
			<div class="span6">
				...
				<small class="customer-assign-btn">
					<input id="search_requester" type="text" placeholder="Search Requester" readonly="readonly" value="<?= $service_unit_details->id_number ?>">
				</small>
				<?php elseif($isAdd): ?>
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						$details_content = get_requester_details($service_unit_details->id_number, "employee");
				...
				<div class="row-fluid">
					<div class="span11">
						<div class="control-group <?= $this->form_validation->error_class('remarks') ?>">
							...
							<div class="controls">
								<?php if(!$isAdd): ?>
								<textarea class='span8' rows="4" placeholder="" name="current_requester_remarks" readonly><?= $service_unit_details->remarks ?></textarea>
								<br/><br/>
			...
			<div class="span5">
				...
				<?php if(!$isAdd):
					...
					if ($service_unit_details->warehouse_id == 0)
				...
				?>
				<?= form_dropdown('add_item_warehouse',$warehouse_options, set_value('add_item_warehouse',$service_unit_details->warehouse_id),'id="add_item_warehouse"') ?>
				<?php elseif($isAdd): 
					$warehouse_options = array();
					$warehouse_options = array('0' => 'Select a Warehouse...');
					foreach ($warehouse_details as $wd) {
				...
				<?php if(!$isAdd):
					...
					echo form_dropdown('add_item_brandmodel',$brand_model_options, set_value('add_item_brandmodel',$service_unit_details->motorcycle_brand_model_id),'id="add_item_brandmodel"');
				?>
				<?php elseif($isAdd):
					$brand_model_options = array();
					$brand_model_options = array('0' => 'Select a Brand/Model...');
				...
				<?php if(!$isAdd): ?>
				<input name="engine" id="engine" class="" placeholder="Enter Engine Number..." value="<?= $service_unit_details->engine ?>" />
				<?php elseif($isAdd): ?>
				...
				<?php if(!$isAdd): ?>
				<input name="chassis" id="chassis" class="" placeholder="Enter Chassis Number..." value="<?= $service_unit_details->chassis ?>" />
				<?php elseif($isAdd): ?>
		...
		<div class="row-fluid">
			...
			<table class="table inventory-order-items table-bordered table-striped">
				...
				<tbody id="wr_items">
					...
					if (!$isAdd) {
						//$temp = array($service_unit_detail_details);
					...
					<tr class="item_row">
						...
						<td id="<?= $temp[$i]['service_unit_detail_id'] ?>" data="<?= $temp[$i]['service_unit_detail_id'] ?>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>
		...
		<div class="controls" align="right">
			<!--a id='submit_order' class="btn btn-primary">Save Service Unit</a-->
...
<script type="text/javascript">
	...
	var item_entry_row = _.template('\
		...
		<td id="<%= active_service_unit_detail_id %>" data="<%= active_service_unit_detail_id %>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>\n\
		<td class="hidden_values">\n\
	...
	$("#add_wr_item").click(function(){
		...
		else
		{
			...
			// ajax request
			b.request({
				url : '/spare_parts/service_unit/create_request',
				data : {				
