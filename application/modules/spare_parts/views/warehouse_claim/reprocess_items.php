...
$submitURL = "/spare_parts/" . $department_module_details->segment_name . "/reprocess/" . $warehouse_claim_details->warehouse_claim_id;
...
<?php
	...
	$details_content = get_requester_details($warehouse_claim_details->id_number, "employee", 1);

	$motor_brand_model_detail = "N/A";
	if ($warehouse_claim_details->motorcycle_brand_model_id > 0) {
		$motor_details = $this->warehouse_model->get_motorcycle_brand_model_class_view_by_id($warehouse_claim_details->motorcycle_brand_model_id);
	...
	<table  class='table table-bordered'>
		...
		<tbody>
			...
			<tr>
				...
				<td><?= $warehouse_claim_details->engine ?></td>
			</tr>	
			<tr>
				<td><strong>POSITION:</strong></td>
				<td><?= $details_content->position_name ?></td>
				<td><strong>CHASSIS:</strong></td>
				<td><?= $warehouse_claim_details->chassis ?></td>
			</tr>	
			<tr>
				<td><strong>EMAIL:</strong></td>
				<td><?= $details_content->email_address ?></td>
				<td><strong>STATUS:</strong></td>
				<td><?= $warehouse_claim_details->status ?></td>
			</tr>	
			<tr>
				...
				<?php
					if (strlen(trim($warehouse_claim_details->remarks)) > 0)
						echo "<td><a href='#' id='view-full-remarks' data='{$warehouse_claim_details->remarks}'><u>View Remarks</u></a></td>";
					else
						echo "<td>N/A</td>";
				?>				
			</tr>								
...
<form id="submit_form" action='<?= $submitURL ?>' method='post' class='form-inline'>
	<fieldset >		
					...
					<tr>
						...
						<!--td>
								...
								foreach ($request_items as $ri) {
									...
									$item_detail_options[$ri->warehouse_claim_detail_id] = $item_view_details->sku . ' - ' . $item_view_details->description;
			...
			<table class="table inventory-order-items table-bordered table-striped">
				...
				<tbody id="wr_items">
					...
					<tr class="item_row">
						...
						<td class ="remove_item" id="<?= $temp[$i]['request_item_id'] ?>" data="<?= $temp[$i]['warehouse_claim_detail_id'] ?>" data-id="<?= $temp[$i]['request_item_id'] ?>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>						
		...
		<div class="controls" align="right">
			...
			<a href='/spare_parts/display_returnslip/<?= $warehouse_claim_details->request_code ?>' target = '_blank' id='print_return_slip' class="btn btn-primary"><span id="prs_caption">Print Return Slip Form</span></a>						
...
<script type="text/javascript">
	...
	var item_entry_row = _.template('\
		...
		<td id="<%= active_reprocessed_item_id %>" data="<%= active_warehouse_claim_detail_id %>" data-id="<%= active_reprocessed_item_id %>"><a class="btn btn-danger rmv_wr_item"><i class="icon-white icon-minus"></i></a></td>\n\
		<td class="hidden_values">\n\
		...
		$("#btn_item_search").click(function(e) {
			...
			b.request({
				...
				data: {
					...
					"request_id": <?= $warehouse_claim_details->warehouse_claim_id ?>,
	...
	$("#add_wr_item").live('click',function(){
		...
		if (_hasError == 1) {
			...
			// ajax request
			b.request({
				url : '/spare_parts/warehouse_claim/proceed_reprocess_item',
				data : {				
	...
	$("#view-full-remarks").click(function(e){
		...
		var request_code = '<?= $warehouse_claim_details->request_code ?>';
	...
	$("#forward_to_warehouse").live('click',function(){
		...
		var request_code = '<?= $warehouse_claim_details->request_code ?>';
		var segment_name = '<?= $department_module_details->segment_name ?>';
		var request_id   = <?= $warehouse_claim_details->warehouse_claim_id ?>;
