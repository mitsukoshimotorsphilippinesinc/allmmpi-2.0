	...
	<table class='table table-condensed table-striped table-bordered' style='font-size:12px'>
		...
		if (empty($all_logs)) {
			...
			foreach($all_logs as $al)
			{
				...
				if ($al->is_corpo_sharing == 1) {
					if (!empty($al->group_name)) {
						$group_tag = "<label class='label label-warning'>{$al->group_name}</label>";
					}
				}	
				...
				$history_html .= "<tr>																
									...
									<td style='text-align:left;'>{$al->cash_card}{$group_tag}</td>  									
									<td style='text-align:center;'>{$payout_period}</td>								
								</tr>";
				if (!(trim($al->remarks) == "")) {				 				
					$history_html .="<tr style='font-size:12px;'>
