...
class Pager {
	 ...
	 */
	public function create_links($type='')
	{
		...
		$pagination .= $this->prev_link($this->template['previous_text'], $type);
		$pagination .= $this->page_links($type);
		$pagination .= $this->next_link($this->template['next_text'], $type);
	...
	public function page_links($type='')
	{
		...
		if ($end < 5 + ($this->adjacents * 2)) 
		{
			...
			for($i = $start; $i <= $end; $i++)
			{
				...
				{
					$pagination .= $this->template['active_start'].anchor('#', $i, array('onclick'=>'return false;')).$this->template['active_end'];
				}
				else
				{
					$url = ($i == 1) ? '' : '/'.$i;
					$pagination .= $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').$url.$type, $i).$this->template['item_end'];
				}
			}
		...
		elseif ($end >= 5 + ($this->adjacents * 2))
		{
			...
			if ($this->current_page < 2 + ($this->adjacents * 2))
			{
				...
				$pagination .= $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').'/'.($end-1).$type, ($end-1)).$this->template['item_end'];
				$pagination .= $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').'/'.$end.$type, $end).$this->template['item_end'];
			}
			elseif ($end - ($this->adjacents * 2) - 1 >= $this->current_page && $this->current_page >= ($this->adjacents * 2))
			{
				// current page somewhere in the middle
				$pagination .= $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').'/'.'1'.$type, '1').$this->template['item_end'];
				$pagination .= $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').'/'.'2'.$type, '2').$this->template['item_end'];
				$pagination .= $this->template['disabled_start'].anchor('#', '...', array('onclick'=>'return false;')).$this->template['disabled_end'];
	 ...
	 */
	public function next_link($value, $type ="")
	{
		...
		if ($this->current_page == $this->total_pages)
		{
			return $this->template['disabled_start'].anchor('#', $value.$this->template['next_mark'], array('onclick'=>'return false;')).$this->template['disabled_end'];
		}
		else
		{
			$next_page = $this->current_page + 1;
			return $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').'/'.$next_page.'/'. $type, $value.$this->template['next_mark']).$this->template['item_end'];
		}
	}
	 ...
	 */
	public function prev_link($value, $type="")
	{
		if ($this->total_pages == 0)
		{
			return '';
		...
		{
			return $this->template['disabled_start'].anchor('#', $this->template['previous_mark'].$value, array('onclick'=>'return false;')).$this->template['disabled_end'];
		}
		else
		{
			$previous_page = $this->current_page - 1;
			$previous_page = ($previous_page == 1) ? '' : '/'.$previous_page;
			return $this->template['item_start'].anchor(rtrim($this->pagination_url, '/').$previous_page.'/'. $type, $this->template['previous_mark'].$value).$this->template['item_end'];
		}
	}
}
