...
class Asset {
	...
	{
		$asset_location = $this->_asset_path($asset_name, $module_name, config_item('asset_css_dir'));
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
	...
	{
		$asset_location = $this->_asset_url($asset_name, $module_name, config_item('asset_css_dir'));
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
		return $asset_location;
	}
	...
	{
		$asset_location = $this->_asset_path($asset_name, $module_name, config_item('asset_img_dir'), 'path');
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
		return $asset_location;
	}
	...
	{
		$asset_location = $this->_asset_url($asset_name, $module_name, config_item('asset_img_dir'));
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
		return $asset_location;
	}
	...
	{
		$asset_location =  $this->_asset_path($asset_name, $module_name, config_item('asset_js_dir'));
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
		return $asset_location;
	...
	{
		$asset_location = $this->_asset_url($asset_name, $module_name, config_item('asset_js_dir'));
		$asset_location .= '?v=' . $this->getfiletime(FCPATH.$asset_location);
		return $asset_location;
	}
	...
	private function _other_asset_location($asset_name, $module_name = NULL, $asset_type = NULL, $location_type = 'url')
	{
		...
		{
			$asset_location = (substr($asset_name, 0, 1) != '/' ? '/' : '').$asset_name;
		}
