	...
	var internal = {
		...
		render: function ( element ) {
			...
			this.realDiv =$( '<div/>', { 'class':'real' } );
				this.fileField = $( '<input/>', { 'class':'file', 'name':'file',  'type':'file', 'size' : '34'} ).css({'height' : '35', 'margin':'0', 'width' : '400'});
				if (this.settings.simpleFile==true) this.fileField.css({'opacity':1}); else {
						this.fakeDiv = $( '<div/>', { 'class':'fake' } );
						this.ftext =  $( '<input/>', { 'type':'text', 'class':'text bev' } ).css({'display':'inline', 'margin-right' : '5px'}); 
						this.fakeDiv.append( this.ftext );
						this.fakeDiv.append( $( '<input/>', { 'type':'button', 'class':'button bev xx', 'value':'Browse'} ).css({'display':'inline', 'height' : '35px', 'margin' : '0', 'margin-bottom' : '2px'}) );
				...
				fform.append($('<br/>'));
					...
					this.submit = $('<input/>', { 'type':'button', 'value':'Upload', 'class':$.browser.msie ? 'ib':'button bev'}).css({'width':'auto','height' : '35px','position':'relative','padding':'6px 20px'}); 
		...
		addFile: function ( e ) {
			...
			if (this.settings.singleUpload == true) {
				this.files.items = [];
				$("div.fileItem").remove();
				this.current = 0;
				this.fileCount = 0;
				this.upInfoDiv.hide();
				this.upInfoDiv.removeClass('result');
				this.upInfoDiv.html("");
				this.upInfoDiv.css({'background-color':''});
				this.upInfoDiv.append( this.filePro);
				this.upInfoDiv.append($( '<img/>', { 'src':this.settings.progressGIF, 'id':'prImg'} ));
				this.upInfoDiv.append(this.totalPro);
			}
		...
		uploadFinished: function ( evt ) {
			...
			$(this.fileListDiv.children('.fileItem')[this.current].children[1]).css({'width':"100%"});
			var currItem = {
				'fileName': this.files.items[this.current].fileName,
				'fileSize': this.files.items[this.current].fileSize,
				'fileType': this.files.items[this.current].type
			};
			this.settings.onUploadFinished(currItem);
		},
		allDone: function () {
			enable = function ( elm ) {
				elm = $(elm);
			...
			var retItems = [];
			for(f in this.files.items){
				var tmpItem = {
					'fileName': this.files.items[f].fileName,
					'fileSize': this.files.items[f].fileSize,
					'fileType': this.files.items[f].type
				};
				retItems.push(tmpItem);
			}
			this.settings.onComplete(retItems);
			enable(this.submit);
			enable(!this.fakeDiv ? this.fileItem: this.fakeDiv.children()[1]);
		},
